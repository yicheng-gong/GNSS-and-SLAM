...
bool Initializer::Initialize(const Frame &CurrentFrame,
  ...
  stringstream result;
  initFrameCounter++;
  result << N << " Frames Count: " + std::to_string(initFrameCounter) + " frames";
  if (success) {
    result << ("\rRequir " + std::to_string(initFrameCounter) + " frames for the tracker to successfully initialize!");
  }
